...
# testStr = resFlagsTeam.at[0, 'opponentVector']
...
# testOneMore = resFlagsTeam.iloc[0:1, [0,1,2,3,4,5,8]]
# print('test zero: ', pd.concat([testOneMore, newDf.transpose()]))
# testOneMore = pd.concat([testOneMore, newDf.transpose()])
# print('test one: ', testOneMore)
# testStr = resFlagsTeam.at[0, 'teamVector']
# newArrayTeam = np.array(testStr.replace('[', '').replace(']', '').split(', ')).astype(np.float)
# newColomn = []
# for index in range(len(newArrayTeam)):
#     newColomn.append(f'Tm{index}')
# newDf = pd.DataFrame(data=newArrayTeam, index=newColomn)
# print('test two: ', pd.concat([testOneMore, newDf.transpose()]))
# testOneMore = pd.concat([testOneMore, newDf.transpose()])
# print('test three: ', testOneMore)
# print('test four: ', resFlagsTeam.iloc[0:1, 9])
# testOneMore = pd.concat([testOneMore, resFlagsTeam.iloc[0:1, 9]])
# print('test five: ', testOneMore)